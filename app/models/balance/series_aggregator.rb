class Balance::SeriesAggregator
  attr_reader :series_list, :favorable_direction, :currency

  def initialize(series_list:, currency:, favorable_direction:)
  ...
  def aggregate
    return empty_series if series_list.empty?

    values_by_date = series_list.each_with_object(Hash.new { |hash, key| hash[key] = [] }) do |series, hash|
    ...
    Series.new(
      ...
      interval: series_list.first.interval,
      values: values,
      favorable_direction: favorable_direction
    )
  end
